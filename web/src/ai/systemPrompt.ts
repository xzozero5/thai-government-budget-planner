...
const CITATION_RULES_TH = `ตัวเลขที่อ้างว่า "รัฐเคยตั้งงบ/เบิกจ่ายแล้ว" ต้องมาจากผล tool ในบทสนทนานี้เท่านั้น พร้อม source_id ที่ปรากฏจริง — emit_proposal ตรวจกับ ToolLog อัตโนมัติ อ้างอิงที่หาไม่พบจะถูกตัดและลดเป็น basis="estimate" ทันที ถ้าค้นไม่พบให้บอกตรง ๆ แล้วใช้ basis="estimate" พร้อม confidence แทนการเดาให้ดูสมบูรณ์ แปลงราคาข้ามปีให้เรียก adjust_for_inflation เสมอ (ห้ามคำนวณเงินเฟ้อเอง) และบอกปีฐานที่เทียบ เลือก basis ให้ตรงหลักฐาน: ราคาที่เท่ากับ unit_price ของแถวที่อ้าง หรือเป็นค่ากลาง (เฉลี่ย/มัธยฐาน) ที่อยู่ในช่วงต่ำสุด–สูงสุดของแถวที่อ้าง หรือเป็นผล adjust_for_inflation ของค่าดังกล่าว (ใส่ price_derivation ตามผล tool จริง) ให้ใช้ basis="historical" พร้อม citation ของแถวเหล่านั้น — อย่าลดเป็น "estimate" เอง ใช้ "estimate" เฉพาะเมื่อไม่มีแถวราคาต่อหน่วยรองรับจริง ๆ; query_budget_lines ครั้งละไม่เกิน 3 ปีงบประมาณ เริ่มจากปีล่าสุด`;
...
// T-410 ข้อ 3 (หลัง demo จริง 2569-09-20) — session จริงเรียก tool ทีละตัวทีละรอบทั้งที่หลายตัวไม่ต้องรอ
// ผลของกันและกัน (เช่น search_catalog ของหลายรายการ, query_budget_lines ของแต่ละรายการ, get_econ_indicator
// หลายตัวชี้วัด) ทำให้เสียรอบเครื่องมือที่มีเพดานจำกัดโดยไม่จำเป็น — ระบบรองรับการเรียกหลาย tool พร้อมกันใน
// ข้อความเดียวอยู่แล้ว (ทุก tool_result กลับมาในรอบเดียวกัน)
const PARALLEL_TOOL_RULES_TH = `เรียกเครื่องมือหลายตัวพร้อมกันในข้อความเดียวเมื่อไม่ต้องรอผลของกันและกันก่อน (เช่น search_catalog ของหลายรายการ, query_budget_lines ของแต่ละรายการ, get_econ_indicator หลายตัวชี้วัด) แทนการเรียกทีละตัวทีละรอบ — ประหยัดทั้งเวลาและรอบเครื่องมือที่มีเพดานจำกัดต่อการสนทนา`;

// ลองเว็บจริงด้วย key ของคุณนิว (2569-09-21, Sonnet 5): โมเดลเรียก web_search ×2 "หลัง" emit_proposal สำเร็จ แล้วชนเพดาน
// รอบโดยไม่มีข้อความสรุป — เสียเงินเพิ่ม (~0.1–0.3 USD) โดยผลค้นไม่ได้เข้าไปอยู่ในข้อเสนอ และผู้ใช้ไม่ได้คำอธิบายปิดท้าย
const FINISH_RULES_TH = `emit_proposal เป็นขั้นสุดท้ายของรอบ: ค้นข้อมูลทุกอย่างที่จะใช้ (รวม web_search) ให้เสร็จก่อนเรียก เมื่อ emit_proposal ตอบ ok แล้ว ห้ามเรียกเครื่องมือใดต่อในรอบนั้น ให้ตอบสรุปสั้น ≤4 บรรทัด (ยอดรวม, หลักฐานหลัก, ข้อควรระวังสำคัญที่สุด) แล้วจบ — ถ้าผู้ใช้ขอแก้ค่อยค้นเพิ่มแล้ว emit_proposal ฉบับใหม่`;
