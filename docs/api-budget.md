...
ตั้งค่ารอบ T-604 ให้ต่ำกว่าที่อนุมัติ เผื่อ request สุดท้ายของแต่ละข้อเกินเพดานได้ 1 request: `--max-usd 1.80` (ledger ของ eval รวมของเดิม 0.2486 → ใช้ใหม่ ≤ ~1.55) `--max-usd-per-case-haiku 0.22 --max-usd-per-case-sonnet 0.40`; ข้อที่งบไม่พอจะเป็น `not_run (budget)`
