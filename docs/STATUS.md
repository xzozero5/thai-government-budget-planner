...
## สถานะปัจจุบัน
- Phase: **4 และ 5 ทำขนานกัน** (คุณนิวสั่ง 2569-09-20) · Phase 3: โค้ดเสร็จ (T-301–T-305, T-307, T-309), **T-306 runner พร้อม + รันจริง 2 ครั้งกับ 1 โจทย์; eval จริง core8 + T-308 เลื่อนไปรันครั้งเดียวตอน Phase 6 (T-604) ตามคำสั่งคุณนิว** — ห้ามเรียก API จริงจนถึงตอนนั้น
- Phase 4 commit แล้ว: T-401 (design docs + copy 600+ ข้อความ), T-402 (primitives 25 ตัว), T-403 (stores + keyHolder/chatController), T-404 (KeyGate/About), T-405 (app shell + chat), T-406 (proposal pane, props-driven), T-407 (citation drawer), T-412 (viz), i18n `t()` · กำลังทำ: ต่อสาย workspace (proposal/citations/viz ↔ stores), T-502 (export dialog + save/load), T-501 รอบแก้ (PDF: main thread ดูภาพแล้วพบหัวตารางซ้ำทุกหน้า/ไม่มี footer/hyphen แปลกปลอม/ชื่อเรื่องทับกัน) · ยังไม่เริ่ม: T-408 ส่วนที่เหลือ, T-411 motion, T-409 e2e, T-410 review, T-503
