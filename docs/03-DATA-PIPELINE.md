...
- **ไฟล์ค้นหา (T-208)**: `catalog/items-slim.json.gz` (~1.97 MB gz: `{schema_version, data_version, items:[{i, key, name, n_lines, years, has_unit_price, low_specificity?, trend?}]}`) + `catalog/search-index.json.gz` (~1.65 MB gz: MiniSearch prebuilt แบบ key_only + `tokenizer_version` + `data_version`) — สร้างโดย `web/scripts/build-search-index.mjs` (ใช้ tokenizer ชุดเดียวกับ browser) ซึ่ง `tgbp publish`/`sample` เรียกผ่าน Node (ต้องมี `web/node_modules`; ข้ามได้ด้วย `--skip-search-index`); สองไฟล์นี้ลง `manifest.files` แต่**ไม่ถูกนับใน `data_version`** (เป็น derived ของ `items.json.gz` — กัน hash ขยับไม่รู้จบ); browser ต้องตรวจ `data_version`/`tokenizer_version` ก่อนใช้ ไม่ตรง → fallback สร้างเอง
...
| V3 | ราชาเทวะ: ผลรวมต่อ (plan, work, budget_group) เทียบ `summary_ocr_raw_data` (= ยอดที่พิมพ์ในเอกสาร) → รายงาน + flag `group_total_mismatch` รายกลุ่ม; **hard เฉพาะยอดรวมทั้งไฟล์ต่าง ≤ 1 %** (ADR-005: ต้นทางเป็น OCR ไม่ตรง 10/38 กลุ่ม) | soft / hard (รวม) |
